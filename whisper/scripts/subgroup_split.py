...
def load_test_data(language, language_code):
    ...
    files = [
        ...
        DATA_DIR / f"nchlt_{language_code}_whisper.json"
    ]
...
def add_metadata(inference_entries, test_entries):
    ...
    for entry in inference_entries:
        meta = meta_data.get(entry['audio'], {})
...
def main():
    ...
    for lang, lang_code in LANGUAGES.items():
        ...
        print("missing age:", sum(e.get("age") in [None, "", "unknown", -1, "-1"] for e in combined_entries))
        print("missing gender:", sum(e.get("gender") in [None, "", "unknown"] for e in combined_entries))
