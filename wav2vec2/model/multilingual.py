...
def load_and_merge(logger, language_codes):
    all_train_datasets = []
    all_test_datasets = []
    ...
    for language_code in language_codes:
        ...
        logger.info(f"Loading {language_codes} datasets...")
        lwazi_ds = load_jsonl_dataset(str(lwazi_train), str(lwazi_test))

        if has_cv:
            cv_ds = load_dataset("json", data_files={"train": str(cv_train)})
            cv_ds["train"] = cv_ds["train"].cast_column("age", Value("string"))
            merged_train = concatenate_datasets([lwazi_ds["train"], cv_ds["train"]])
        else:
            merged_train = lwazi_ds["train"]

        all_train_datasets.append(merged_train)
        all_test_datasets.append(lwazi_ds["test"])  # test sets stay language-specific
...
def train_model(language, language_codes, logger):
    """
    ...
    dataset = load_and_merge(logger, language_codes)
    dataset = dataset.map(speech_file_to_array_fn)
    dataset = dataset.map(prepare_dataset)
    dataset["train"] = dataset["train"].select(range(1000))  # Limit to 1500 samples for quick training
    dataset["test"] = dataset["test"].select(range(200))  # Limit to 250 samples for quick evaluation
    ...
    output_dir = BASE_DIR / f"model/nguni_multilingual_{'_'.join(language_codes)}_wav2vec2"
    logger.info(f"Multilingual training on: {', '.join(language_codes)}")
    ...
    add_safe_globals({
        np.ndarray: True,
        np.dtype: True,
        np.core.multiarray._reconstruct: True,
        np.uint32: True,
    })
    ...
    def patched_torch_load(f, *args, **kwargs):
        if isinstance(f, str) and "rng_state.pth" in f:
    ...
    # Compute WER
    metrics_path = BASE_DIR / f"results/{', '.join(language_codes)}_wav2vec2/metrics.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--languages", nargs='+', type=str, required=True, 
                        help="Language to train the model on  (e.g., 'zu', 'xh', 'ssw', 'nbl')")
    args = parser.parse_args()
    logger = setup_logging(args.languages)
    logger.info(f"Starting training for {args.languages} languages.")
    train_model(args.languages, args.languages, logger)
    logger.info(f"Training completed for {args.languages} languages.")